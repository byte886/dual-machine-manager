...
> 这里只记"装了什么、常态端口"用于快速预判；**真正使用时必须现场探测 + 连通自测**（探测动作见 mac-system-toolkit `vpn-control.md`，代理使用原则由全局 `AGENTS.md` 第三章路由统一加载，本篇不回指上层方法论）。端口会随客户端设置或开关变化，**不得把台账值写死进脚本**。订阅地址/账号属敏感凭证，须加密落盘、不放本表。
